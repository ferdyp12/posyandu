...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="card shadow">
            <div class="card-body">
                <?= form_open('', ['id' => 'form-update-jenisimunisasi']); ?>
...
<?= $this->section('script'); ?>
<script>
    $('#nama').val('<?= $imunisasi->nama; ?>')
    $('#singkatan').val('<?= $imunisasi->singkatan; ?>')

    let csrfToken = '<?= csrf_token(); ?>'
    let csrfHash = '<?= csrf_hash(); ?>'

    $('#form-update-jenisimunisasi input').on('keyup', function() {
        $.ajax({
            method: 'POST',
    ...
    $('#form-update-jenisimunisasi').on('submit', function(event) {
