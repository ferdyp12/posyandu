...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="card shadow">
            <div class="card-body">
                <?= form_open('', ['id' => 'form-update-timbang']); ?>
                <?= view('timbang/_form'); ?>
                <?= form_close(); ?>
            </div>
        </div>
...
<?= $this->section('script'); ?>
<script>
    $('#id_anak').val('<?= $timbang->id_anak; ?>')
    $('#id_anak').attr('disabled', true)
    $('#tanggal').val('<?= $timbang->tanggal; ?>')
    $('#berat_badan').val('<?= $timbang->berat_badan; ?>')
    $('#tinggi_badan').val('<?= $timbang->tinggi_badan; ?>')
    $('#keterangan').val('<?= $timbang->keterangan; ?>')

    let csrfToken = '<?= csrf_token(); ?>'
    let csrfHash = '<?= csrf_hash(); ?>'

    $('#form-update-timbang .form-input').on('keyup', function() {
        $.ajax({
            method: 'POST',
    ...
    $('#form-update-timbang').on('submit', function(event) {
